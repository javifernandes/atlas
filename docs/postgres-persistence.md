...
Migration 009 adds reversible closed-Session archive state and a backfilled `last_activity_at`
summary. New merged-PR activity advances that maximum transactionally, so bounded page reads can
show exact Session recency without scanning or fully hydrating activity history.
